...
class TestLegalityChecks:
    ...
    def test_non_allowlisted_command_blocked_in_safe_mode(self):
        """Test non-allowlisted commands are blocked without privileged permission."""
        from tascer.overlord.legality import check_action_legality

        result = check_action_legality(
            action_id="terminal.run",
            inputs={"command": "curl https://example.com"},
            permissions={"terminal"},
        )

        assert not result.is_legal
        assert any("allowlisted" in v.lower() for v in result.violations)

    def test_non_allowlisted_command_allowed_with_terminal_unrestricted(self):
        """Test privileged permission can bypass command allowlist."""
        from tascer.overlord.legality import check_action_legality

        result = check_action_legality(
            action_id="terminal.run",
            inputs={"command": "curl https://example.com"},
            permissions={"terminal", "terminal_unrestricted"},
        )

        assert result.is_legal

    def test_shell_chaining_blocked_in_safe_mode(self):
        """Test shell metacharacter chaining is blocked in safe mode."""
        from tascer.overlord.legality import check_action_legality

        result = check_action_legality(
            action_id="terminal.run",
            inputs={"command": "ls | wc -l"},
            permissions={"terminal"},
        )

        assert not result.is_legal
        assert any("metacharacters" in v.lower() for v in result.violations)
