...
def cmd_run(args):
    ...
    privileged_enabled = os.environ.get("TASCER_CLI_PRIVILEGED") == "1"
    permissions = {"terminal"}
    if args.force and privileged_enabled:
    ...
    result = check_action_legality(
        ...
        has_checkpoint=args.force and privileged_enabled,
    )

    if not result.is_legal:
        print(f"🚫 BLOCKED: {result.violations[0]}")
        if not args.force:
            print("   Use --force with TASCER_CLI_PRIVILEGED=1 to override")
            return 1
        if not privileged_enabled:
            print("   Set TASCER_CLI_PRIVILEGED=1 to enable privileged overrides.")
            return 1
        print("   Proceeding with privileged override...")
    ...
    output = run_and_observe(command_parts, shell=False, timeout_sec=args.timeout)
...
def cmd_sandbox_exit(args):
    ...
    if result.success:
        if action == "commit":
            print("✅ Sandbox changes committed")
