...
Persistence:
- Neural links persist to SQLite database
- Survives restarts - true cross-session memory
- Hebbian learning accumulates over time
"""
...
class NeuralMemory:
    ...
    Persistence:
    - Automatically saves to SQLite database
    - Loads existing neural network on startup
    - Cross-session memory accumulation
    """

    # Default database location
    DEFAULT_DB_PATH = Path.home() / ".claude-mem" / "neural.db"

    def __init__(self, db_path: Optional[Path] = None, auto_load: bool = True):
    ...
    def _ensure_schema(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.executescript("""
                -- Neural links between cards (synapses)
                CREATE TABLE IF NOT EXISTS neural_links (
                    ...
                    last_activated TEXT,
                    potentiation_rate REAL DEFAULT 0.15,
                    depression_rate REAL DEFAULT 0.02,
                    created_at TEXT DEFAULT CURRENT_TIMESTAMP,
                    PRIMARY KEY (source_id, target_id)
                );

                -- Neural card stats (neuron properties)
                CREATE TABLE IF NOT EXISTS neural_cards (
                    card_id INTEGER PRIMARY KEY,
                    recall_count INTEGER DEFAULT 0,
                    last_recalled TEXT,
                    resting_potential REAL DEFAULT 0.0,
                    activation_threshold REAL DEFAULT 0.3
                );
                ...
                CREATE TABLE IF NOT EXISTS neural_queries (
                    ...
                    query TEXT,
                    recalled_ids TEXT,  -- JSON array
                    timestamp TEXT DEFAULT CURRENT_TIMESTAMP
                );

                -- Indexes for fast lookups
                CREATE INDEX IF NOT EXISTS idx_links_source ON neural_links(source_id);
                CREATE INDEX IF NOT EXISTS idx_links_target ON neural_links(target_id);
                CREATE INDEX IF NOT EXISTS idx_links_strength ON neural_links(strength DESC);
            """)
    ...
    def _load_from_db(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # Load card stats
            for row in conn.execute("SELECT * FROM neural_cards"):
                card_id = row["card_id"]
                if card_id in self.cards:
                    self.cards[card_id].recall_count = row["recall_count"]
                    if row["last_recalled"]:
                        self.cards[card_id].last_recalled = datetime.fromisoformat(row["last_recalled"])

            # Load links
            for row in conn.execute("SELECT * FROM neural_links"):
                link = NeuralLink(
                    source_id=row["source_id"],
                    target_id=row["target_id"],
                    strength=row["strength"],
                    activation_count=row["activation_count"],
                    potentiation_rate=row["potentiation_rate"],
                    depression_rate=row["depression_rate"],
                )
                if row["last_activated"]:
                    link.last_activated = datetime.fromisoformat(row["last_activated"])

                key = (row["source_id"], row["target_id"])
                self.links[key] = link

                # Wire up to cards if they exist
                if row["source_id"] in self.cards:
                    self.cards[row["source_id"]].outgoing[row["target_id"]] = link
                if row["target_id"] in self.cards:
                    self.cards[row["target_id"]].incoming[row["source_id"]] = link
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            for (src, tgt), link in self.links.items():
                conn.execute("""
                    ...
                    (source_id, target_id, strength, activation_count, last_activated,
                     potentiation_rate, depression_rate)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                """, (
                    src, tgt, link.strength, link.activation_count,
                    link.last_activated.isoformat(),
                    link.potentiation_rate, link.depression_rate
                ))

            # Save card stats
            for card_id, card in self.cards.items():
                conn.execute("""
                    INSERT OR REPLACE INTO neural_cards
                    (card_id, recall_count, last_recalled, resting_potential, activation_threshold)
                    VALUES (?, ?, ?, ?, ?)
                """, (
                    card_id, card.recall_count,
                    card.last_recalled.isoformat() if card.last_recalled else None,
                    card.resting_potential, card.activation_threshold
                ))

            conn.commit()

    def save_link(self, link: NeuralLink):
        """Save a single link (for incremental saves)."""
        with sqlite3.connect(self.db_path) as conn:
            conn.execute("""
                INSERT OR REPLACE INTO neural_links
                (source_id, target_id, strength, activation_count, last_activated,
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (
                link.source_id, link.target_id, link.strength, link.activation_count,
                link.last_activated.isoformat(),
                link.potentiation_rate, link.depression_rate
            ))
            conn.commit()
    ...
    def get_stats(self) -> Dict:
        ...
        with sqlite3.connect(self.db_path) as conn:
            link_count = conn.execute("SELECT COUNT(*) FROM neural_links").fetchone()[0]
            card_count = conn.execute("SELECT COUNT(*) FROM neural_cards").fetchone()[0]
            query_count = conn.execute("SELECT COUNT(*) FROM neural_queries").fetchone()[0]
            strong_links = conn.execute(
                "SELECT COUNT(*) FROM neural_links WHERE strength > 0.5"
            ).fetchone()[0]

        return {
            "persisted_links": link_count,
            "persisted_cards": card_count,
    ...
    def connect(self, source_id: int, target_id: int,
                initial_strength: float = 0.1, persist: bool = True) -> NeuralLink:
    ...
    def recall(self, query: str, top_k: int = 10) -> List[Tuple[int, float]]:
        ...
        # Track query
        recalled_ids = [r[0] for r in results[:top_k]]
        self.query_history.append((query, recalled_ids, datetime.utcnow()))

        # Persist query to database
        self._save_query(query, recalled_ids)

        return results[:top_k]

    def _save_query(self, query: str, recalled_ids: List[int]):
        """Save query to database for pattern analysis."""
        import json
        with sqlite3.connect(self.db_path) as conn:
            conn.execute("""
                INSERT INTO neural_queries (query, recalled_ids, timestamp)
                VALUES (?, ?, ?)
            """, (query, json.dumps(recalled_ids), datetime.utcnow().isoformat()))
            conn.commit()
